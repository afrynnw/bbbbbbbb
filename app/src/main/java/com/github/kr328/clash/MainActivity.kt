...
class MainActivity : BaseActivity<MainDesign>() {
    override suspend fun main() {
        // Auto-import bundled config on first run
        firstRunSetup()
    ...
    private suspend fun firstRunSetup() {
        val prefs = getSharedPreferences("tii_setup", MODE_PRIVATE)
        ...
        try {
            ...
            for (i in 0 until 10) {
                ...
                try {
                    withProfile {
                        val uuid = create(Profile.Type.File, "Tii Default", "")
                        val configFile = File(filesDir, "pending/$uuid/config.yaml")
                        assets.open("default.yaml").bufferedReader().use {
            ...
            if (ok) prefs.edit().putBoolean("configured", true).apply()
        } catch (_: Exception) {}
    }
